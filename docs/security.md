...
API-key rate limiting uses layered defense in depth: an in-memory limit is always checked, and a Redis-backed limit is also checked when Redis is configured. Redis is not the exclusive limiter.
